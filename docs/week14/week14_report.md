...
### 2-4. 멀티브랜치 파이프라인 및 자동 트리거(SCM 폴링)

- **멀티브랜치 파이프라인**: JCasC에 `multibranchPipelineJob`을 git source로 정의하였다. `git ls-remote` 기반이라 GitHub API rate limit이나 토큰 없이 모든 브랜치를 발견한다. main 브랜치를 자동 발견하여 빌드 SUCCESS하였고, 시연용 테스트 브랜치(`ci/jenkins-multibranch-demo`)를 push한 뒤 재스캔하자 새 브랜치를 자동 발견하여 별도 빌드 SUCCESS까지 확인하였다. 즉 브랜치별 독립 빌드가 실제로 동작한다.
- **자동 트리거**: GitHub Webhook이 실제로 동작하려면 로컬 Jenkins를 공인 URL로 외부 노출해야 하는데, 현재 컨테이너는 root 권한 + docker.sock 마운트 + 기본 비밀번호 상태라 공개 인터넷 노출은 보안상 부적절하다. 따라서 외부 노출 없이 동작하는 **SCM 폴링**을 채택하였다. 단일 pipelineJob에는 `H/2 * * * *` 폴링 트리거를, 멀티브랜치에는 `periodicFolderTrigger(2m)`를 설정하여 push 후 수 분 내 자동 빌드되도록 하였다.
- **Slack 알림**: Jenkinsfile의 post 단계에 `slackSend`를 조건부(SLACK_CHANNEL 설정 시)로 작성해 두었다. 봇 토큰/Webhook URL 발급이 필요하여 실제 발송 검증은 범위에서 제외하였다.
- **운영상 발견**: 멀티브랜치 job-dsl은 JCasC reload API로는 적용되지 않고(파싱 단계에서 실패), 컨테이너 재시작 시의 초기 JCasC 로드에서만 정상 생성된다는 점을 확인하였다.
...
멀티브랜치 + 자동 트리거 동작 검증: 멀티브랜치 파이프라인이 main과 테스트 브랜치 2개를 자동 발견하여 각각 빌드 SUCCESS하였다. SCM 폴링 트리거(H/2)를 설정하여 외부 노출 없이 push 후 자동 빌드되는 구조를 구성하였다.
...
멀티브랜치 파이프라인으로 브랜치별 자동 빌드를 실제로 시연하였다. 특히 git source가 GitHub API 대신 git ls-remote를 사용해 rate limit 없이 브랜치를 발견한다는 점, 그리고 멀티브랜치 job-dsl이 JCasC reload로는 적용되지 않아 컨테이너 재시작이 필요하다는 점을 디버깅을 통해 직접 확인하였다. push 후 자동 트리거는 보안(외부 노출 위험)을 고려해 Webhook 대신 SCM 폴링으로 구현하였다.

아쉬운 점은 GitHub Webhook의 즉시 트리거와 Slack 실제 발송까지는 검증하지 못한 것이다. 전자는 root + docker.sock 상태의 로컬 Jenkins를 공개 인터넷에 노출하는 보안 위험 때문에 의도적으로 SCM 폴링으로 대체하였고, 후자는 토큰/URL 발급이 필요해 범위에서 제외하였다. 두 기능 모두 설정은 코드로 작성해 두어, 권한이 분리된 공개 서버와 토큰이 확보되면 즉시 활성화 가능하다.
